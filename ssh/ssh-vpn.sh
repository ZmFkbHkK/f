...
<html>
<kepala>
<meta http-equiv="REFRESH" content="0;url=https://wa.me/6285935195701">
</kepala>
<tubuh>
<p>Pengalihan URL</p>
</tubuh>
...
wget -q -O /etc/systemd/system/badvpn.service "${REPO}ssh/badvpn.service" >/dev/null 2>&1
...
cat >/etc/cron.d/exp <<-END
SHELL=/bin/sh
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin
58 23 * * * root /usr/sbin/killusr exp
END

cat >/etc/cron.d/logclean <<-END
SHELL=/bin/sh
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin
*/10 * * * * root /usr/sbin/m-set clearlog
END

cat >/etc/cron.d/daily_reboot <<-END
SHELL=/bin/sh
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin
0 3 * * * root /usr/sbin/reboot
END
